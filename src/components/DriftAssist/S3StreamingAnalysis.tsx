...
=======
      // Don't set analysisComplete here; let the useEffect handle it!
>>>>>>> f0ce0ac (das)
    } catch (error) {
...
<<<<<<< HEAD
=======
   * Check if all selected resources are completed and set analysisComplete
   */
  useEffect(() => {
    if (
      analysisData &&
      analysisData.selectedResources &&
      analysisData.selectedResources.length > 0
    ) {
      const allDone = analysisData.selectedResources.every(resourceId => {
        const result = resourceResults[resourceId];
        return result && result.status === 'completed';
      });
      if (allDone && !analysisComplete) {
        setAnalysisComplete(true);
      }
    }
  }, [resourceResults, analysisData, analysisComplete, setAnalysisComplete]);
