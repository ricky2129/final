...
import { Metrics } from "themes";
import { useGetAllDeployments } from "react-query/traceAssistQueries";
import { useParams } from "react-router-dom";
...
=======
  const { data: deployments, isLoading, isError } = useGetAllDeployments(project_id, application_id);

  return (
    <Flex vertical gap={Metrics.SPACE_LG}>
      <h2>Deployments</h2>
      {isLoading && <Spin tip="Loading deployments..." />}
      {isError && <Alert type="error" message="Failed to load deployments." />}
      {!isLoading && !isError && (
        <ul>
          {deployments && deployments.length > 0 ? (
            deployments.map((deployment) => (
              <li key={deployment.id || deployment.deployment_name}>
                {deployment.deployment_name} ({deployment.repo_url})
              </li>
            ))
          ) : (
