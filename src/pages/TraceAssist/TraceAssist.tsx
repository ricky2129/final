...
import { Table, Spin, Empty, Tag, Button, message, Descriptions } from "antd";
import { useParams } from "react-router-dom";
import { useGetAllDeployments, useInstrumentDeployment, useSetupOtelComponents } from "react-query/traceAssistQueries";
import "./TraceAssist.styles.scss";
...
=======
  ...
  const {
    mutate: instrumentDeployment,
  } = useInstrumentDeployment();
  const {
    mutate: setupOtelComponents,
    isLoading: isSetupOtelLoading,
  } = useSetupOtelComponents();

  const [instrumentingDeployment, setInstrumentingDeployment] = useState<string | null>(null);
  const [selectedDeployment, setSelectedDeployment] = useState<any | null>(null);

  const [isOtelButtonLoading, setIsOtelButtonLoading] = useState(false);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!instrumentingDeployment || !deployments) return;
    const deployment = deployments.find((d) => d.name === instrumentingDeployment);
    if (deployment && deployment.is_instrumented) {
      setInstrumentingDeployment(null);
      message.success(`Deployment "${deployment.name}" is now instrumented!`);
    }
  }, [deployments, instrumentingDeployment]);

  useEffect(() => {
    if (!selectedDeployment || !deployments) return;
    const updated = deployments.find(d => d.name === selectedDeployment.name);
    if (updated && updated !== selectedDeployment) {
      setSelectedDeployment(updated);
    }
  }, [deployments, selectedDeployment]);

  const filteredDeployments =
    deployments?.filter((dep) => dep.namespace !== "kube-system") ?? [];
  ...
  const columns = [
    ...
    {
      title: "Instrumented",
      dataIndex: "is_instrumented",
      key: "is_instrumented",
      render: (val: boolean, record: any) =>
        val ? (
          <Tag color="green">Instrumented</Tag>
        ) : (
          <Button
            type="primary"
            size="small"
            loading={instrumentingDeployment === record.name}
            disabled={
              instrumentingDeployment === record.name ||
              record.is_instrumented === true
            }
            onClick={(e) => {
              e.stopPropagation();
              setInstrumentingDeployment(record.name);
              instrumentDeployment(
                {
                  deployment_name: record.name,
                  namespace: record.namespace,
                },
                {
                  onSuccess: () => {
                    message.success("Instrumentation started!");
                    refetch();
                  },
                  onError: (error: any) => {
                    message.error(
                      error?.message || "Failed to start instrumentation."
                    );
                    setInstrumentingDeployment(null);
                  },
                }
  ...
  const title = (
    <div style={{ textAlign: "center", marginBottom: 24 }}>
      <h2 style={{ margin: 0 }}>Trace Assist</h2>
    </div>
  );

  if (isLoading) {
    return (
      <div className="trace-assist-container" style={{ textAlign: "center", marginTop: 40 }}>
        {title}
        <Spin size="large" />
        <div style={{ marginTop: 16, fontSize: 16 }}>
          Please wait, getting the deployment details.
        </div>
>>>>>>> f0ce0ac (das)
      </div>
    );
...
=======
  if (isError || filteredDeployments.length === 0) {
    return (
      <div className="trace-assist-container" style={{ textAlign: "center", marginTop: 40 }}>
        {title}
  ...
  const otelSetupRow = (
    ...
    >
      <span style={{ fontWeight: 500 }}>Setup OpenTelemetry Stack:</span>
      <Button
        type="primary"
        loading={isOtelButtonLoading}
        disabled={isOtelButtonLoading}
        onClick={() => {
          setIsOtelButtonLoading(true);
          setupOtelComponents(undefined, {
            onSuccess: (data) => {
              message.success(
                data?.message || "OpenTelemetry stack setup started!"
              );
              setIsOtelButtonLoading(false);
            },
            onError: (error: any) => {
              message.error(
                error?.message || "Failed to setup OpenTelemetry stack."
              );
              setIsOtelButtonLoading(false);
            },
          });
      ...
      >
        {isOtelButtonLoading ? "Setting up..." : "Setup OTEL"}
      </Button>
    </div>
  );
  const renderMetricsPanels = (panel_links) => {
    if (!panel_links || panel_links.length < 4) {
      return (
        <Empty
          description="Metrics panels are not available."
          style={{ marginTop: 24 }}
        />
      );
    }
    ...
    return (
      <div>
        <div style={{ display: "flex", gap: "24px", marginBottom: "24px" }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500, marginBottom: 8 }}>CPU Usage</div>
            <iframe
              src={panel_links[0]}
              title="CPU Usage"
              width="100%"
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500, marginBottom: 8 }}>Memory Usage</div>
            <iframe
              src={panel_links[1]}
              title="Memory Usage"
              width="100%"
        ...
        <div style={{ marginBottom: "24px" }}>
          <div style={{ fontWeight: 500, marginBottom: 8 }}>Latency</div>
          <iframe
            src={panel_links[2]}
            title="Latency"
            width="100%"
        ...
        <div>
          <div style={{ fontWeight: 500, marginBottom: 8 }}>Traces</div>
          <iframe
            src={panel_links[3]}
            title="Traces"
            width="100%"
  ...
  return (
    <div className="trace-assist-container">
      ...
      <Table
        ...
        bordered
        onRow={(record) => ({
          onClick: () => setSelectedDeployment(record),
        })}
      />
    
      {otelSetupRow}
      {selectedDeployment && (
        <div className="trace-assist-details-container" style={{ maxWidth: 900, margin: "40px auto", padding: 24, background: "#fff", borderRadius: 8 }}>
          <h2 style={{ marginBottom: 24 }}>Deployment Details: {selectedDeployment.name}</h2>
          <Descriptions column={1} bordered>
            ...
            <Descriptions.Item label="Instrumented">
              {selectedDeployment.is_instrumented ? (
                <Tag color="green">Instrumented</Tag>
              ) : (
                <Tag color="red">Not Instrumented</Tag>
              )}
            </Descriptions.Item>
          </Descriptions>
          <div style={{ marginTop: 32 }}>
            <h3>Metrics Panels</h3>
            {selectedDeployment.is_instrumented ? (
              renderMetricsPanels(selectedDeployment.panel_links)
            ) : (
              <Empty
                description="Please instrument your deployment to view metrics."
                style={{ marginTop: 24 }}
              />
            )}
          </div>
