...
=======
  const utilizationLabel = useMemo(() => {
    const expName = getChaosDataQuery?.data?.result?.experiment_name || "";
    if (expName.toLowerCase().includes("memory")) {
      return "Free Memory";
    }
    if (expName.toLowerCase().includes("cpu")) {
      return "Server/Hardware Utilization";
    }
    return "Server/Hardware Utilization";
  }, [getChaosDataQuery?.data?.result?.experiment_name]);
